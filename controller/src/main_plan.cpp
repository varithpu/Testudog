...
int main(int argc, char **argv){
	...
	ros::init(argc, argv, "gazebo_controller_plan");
    ros::NodeHandle nh;
	...
	ros::Subscriber sub_goal = nh.subscribe("trajectory", 1, &robot::Testudog::get_traj, &test);
	ros::Publisher pub_error = nh.advertise<geometry_msgs::Point>("tracking_error", 10);
    ...
    while(ros::ok()){
		...
		msg_error.x = test.get_x_error();
		msg_error.y = test.get_y_error();
